...
class Discriminator1(nn.Module):
    ...
    def __init__(self, in_ch, clsnum, mid_ch, normtype='IN', dor=0.1):
        ...
        self.le1 = md.ConvGLU1D(in_ch, mid_ch, 7, 1, normtype)
        self.le2 = md.ConvGLU1D(mid_ch, mid_ch, 6, 2, normtype)
        self.le3 = md.ConvGLU1D(mid_ch, mid_ch, 6, 2, normtype)
        self.le4 = md.ConvGLU1D(mid_ch, mid_ch, 3, 1, normtype)
        self.le_adv = nn.Conv1d(mid_ch, 1, 3, stride=1, padding=(3-1)//2, bias=False)
        self.le_cls = nn.Conv1d(mid_ch, clsnum, 3, stride=1, padding=(3-1)//2, bias=False)
